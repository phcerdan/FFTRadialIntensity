...
RadialTabWidget::RadialTabWidget(QWidget *parent) :
    QMainWindow(parent)
{
    //UI
    {
        ...
        this->setWindowTitle(QApplication::translate("this", "RadialTabWidget", Q_NULLPTR));
        ...
        // this->addDockWidget(static_cast<Qt::DockWidgetArea>(1), dockWidget_9);
        qvtkWidgetInput = new QVTKWidget();
        ...
        dockWidgetInput->setWidget(qvtkWidgetInput);
        {
            ...
            this->qvtkWidgetInput->GetRenderWindow()->GetInteractor()->SetInteractorStyle(style);
        }

        qvtkWidgetFFT = new QVTKWidget();
        ...
        dockWidgetFFT->setWidget(qvtkWidgetFFT);
        {
            ...
            this->qvtkWidgetFFT->GetRenderWindow()->GetInteractor()->SetInteractorStyle(style);
        }

        qvtkWidgetPlot = new QVTKWidget();
    ...
    // Layout: -> not really, use this->splitDockWidget(d1,d2,Qt::Vertical/Horizontal)
...
void RadialTabWidget::SetInput2D(std::string input_file)
{
    ...
    auto renderer = this->qvtkWidgetInput->GetRenderWindow()->GetRenderers()->GetFirstRenderer();
...
{
    this->shiftedFFT2D = imaging::fft_and_shift<Image2DType>( this->inputImage2D );
    this->magnitudeShiftedFFT2D = imaging::modulus_square_fft<ComplexImage2DType>(this->shiftedFFT2D);
    // Scale for visualization
    typedef itk::LogImageFilter<Image2DType, Image2DType> LogFilterType;
    auto logFilter = LogFilterType::New();
    logFilter->SetInput(magnitudeShiftedFFT2D);
    logFilter->Update();
    typedef itk::MinimumMaximumImageCalculator<Image2DType> ImageCalculatorFilterType;
    auto minMaxCalculator = ImageCalculatorFilterType::New();
    minMaxCalculator->SetImage(logFilter->GetOutput());
    minMaxCalculator->ComputeMaximum();
    typedef itk::IntensityWindowingImageFilter<Image2DType, Image2DType > WindowingFilter;
    ...
    auto actor = vtkSmartPointer<vtkImageActor>::New();
    actor->GetMapper()->SetInputData(connector->GetOutput());
    auto renderer = this->qvtkWidgetFFT->GetRenderWindow()->GetRenderers()->GetFirstRenderer();
    renderer->RemoveAllViewProps();
    renderer->AddActor(actor);
    renderer->ResetCamera();
...
{
    using FrequencyImageIterator = itk::FrequencyShiftedFFTLayoutImageRegionConstIteratorWithIndex<Image2DType>;
    const auto intensities_histo = radial_intensity::ComputeRadialFrequencyIntensities<Image2DType, FrequencyImageIterator>(this->magnitudeShiftedFFT2D);
    const auto & avg_intensities = radial_intensity::AverageRadialFrequencyIntensities(std::get<0>(intensities_histo));
  ...
  //   view_[find_degree->first]->GetScene()->RemoveItem(static_cast<unsigned int> (0));
  //   view_[find_degree->first]->GetScene()->AddItem(chart);
